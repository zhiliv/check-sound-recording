<template>
  <template :key="modal.formUuid" v-for="modal in modals">
    <div class="modal-mask select-none" v-show="modal.isShow && modal.form">
      <div :class="{ 'modal-wrapper': !modal.isDrawer }">
        <div
          ...
          :style="{ width: modal.data.options?.width || '50%' }" class="modal-container border-2 border-zinc-600">
          <div class="body-modal max-h-full">
            ...
            <div class="overflow-y-auto pb-4 pl-2 pr-2 min-h-[60px]" ref="subForm">
              <component :is="modal.form" @data="getData" @valid="getValid" v-model="modal.valueModel" />
            </div>
          </div>
          <div :class="{ 'h-15': modal.isDrawer, 'h-16': !modal.isDrawer }" class="bottom-0 p-2 w-full border-t-2 border-neutral-400 bg-neutral-100">
            <app-button @click="close" class="m-1 standart btn-sm btn-error text-slate-100 ring ring-red-100 hover:bg-rose-700"
              v-if="modal.buttons.cancel">Отменить</app-button>
            <app-button :disabled="disabled.save" @click="onSave" :class="{ ring: !disabled.save, 'ring-green-100': !disabled.save }"
              class="btn-success m-1 standart btn-sm float-right text-slate-100 hover:bg-green-700" v-if="modal.buttons.save">Сохранить</app-button>
            <app-button :disabled="disabled.add" @click="onAdd" class="btn-primary m-1 standart btn-sm float-right text-slate-100"
              v-if="modal.buttons.add">Добавить</app-button>
            <app-button @click="close" class="w-20 m-1 standart btn-sm btn-error" v-if="modal.buttons.no">Нет</app-button>
            <app-button @click="onYes" class="w-20 btn-success m-1 standart btn-sm float-right text-slate-100" v-if="modal.buttons.yes">Да</app-button>
            <app-button @click="onSave" class="w-25 btn-success m-1 standart btn-sm float-right text-slate-100" v-if="modal.buttons.change">Изменить</app-button>
          </div>
        </div>
      </div>
    </div>
